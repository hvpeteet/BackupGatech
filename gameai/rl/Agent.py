...
class Agent:
	...
	def updateVtable(self, newState, lastState, action, reward, terminal, availableActions):
		future = 0
		old = self.v_table[lastState][action]
		if not terminal:
			future = self.gamma * max(self.v_table[newState])
		delta = self.learningRate * (reward + future - old)
		self.v_table[lastState][action] += delta
		return None
	...
	def egreedy(self, observation):
		...
		if random.random() < self.epsilon:
			return random.choice(observation.availableActions)
		else:
	...
	def greedy(self, observation):
		...
		# YOUR CODE GOES BELOW HERE
		row = self.v_table[self.calculateFlatState(observation.worldState)]
		max_value = max(row)
