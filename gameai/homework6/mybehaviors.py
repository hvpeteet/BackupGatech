...
def treeSpec(agent):
	...
	# For when I forget which side is which
	# spec = [Selector, KillHero]
	spec = [Selector, 
		[HeroNotInRangeDaemon, 
			[Selector, 
				(Retreat, 0.5, "retreat [from minions]"), 
				[(HitpointDaemon, 0.5, "hp daemon"), 
					[(Sequence, "sequence"), 
						(ChaseMinion, "chase minion"), (KillMinion, "kill minion"),
					],
				],
			],
		],
		...
		# [(HitpointDaemon, 0.9), (KillHero, "kill hero")],
		# (MoveToTarget, (agent.world.getBaseForTeam(agent.getTeam()).getLocation()), "flee from hero"),
		(Retreat, 0.7, "retreat [from hero]"), 
		[(HitpointDaemon, 0.7), (KillHero, "kill hero")],
	]
	#spec = [Selector, (KillHero, "unintentional fight"), [(BuffDaemon, 5), [(HitpointDaemon, 0.5), [Sequence, (ChaseHero, "chasing hero"), (KillHero, "killing hero")]]], (Retreat, 0.5, "retreat"), [(Sequence, "kill minions"), ChaseMinion, KillMinion]]
...
class MoveToTarget(BTNode):
	...
	def execute(self, delta = 0):
		...
		if self.target == None:
			...
			# executing
			self.shootAnyNearbyEnemies()
			return None
		return ret

	def shootAnyNearbyEnemies(self):
		enemy = None
		enemies = self.agent.world.getEnemyNPCs(self.agent.getTeam())
		if len(enemies) > 0:
			for e in enemies:
				d = distance(self.agent.getLocation(), e.getLocation())
				if isinstance(e, Hero) and d < BIGBULLETRANGE + e.getRadius():
					enemy = e
				if enemy is None and isinstance(e, Minion) and d < BIGBULLETRANGE:
					enemy = e

		if self.agent is not None and enemy is not None:
...
### DodgeHeroShots
###
### Dodge shots from hereos.
### Parameters:
###   0: node ID string (optional)


class DodgeHeroShots(BTNode):
	...
	def execute(self, delta = 0):
		...
		if (not self.agent.canDodge()) or distance(self.agent.getLocation(), self.target.getLocation()) > BIGBULLETRANGE:
			return False
		self.shootAtTarget()
		angle = 90
		print("dodge")
...
# Fails when a minon is in range
class HeroNotInRangeDaemon(BTNode):
	...
	def execute(self, delta = 0):
		...
		for e in enemies:
			if isinstance(e, Hero):
				hero = e
				break
		if hero is not None and distance(self.agent.getLocation(), hero.getLocation()) <= BIGBULLETRANGE + 100:
			# fail check
			print "exec", self.id, "fail"
			return False
		else:
