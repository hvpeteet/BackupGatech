...
class Sequence(BTNode):
	...
	### If a child requires several ticks to complete execution, then the child w\ill return None. If a child returns none, the sequence also returns None.
	### IF a sequence node has no children, it succeeds automatically.
	def execute(self, delta = 0):
		BTNode.execute(self, delta)
		### YOUR CODE GOES BELOW HERE ###
		ret = self.getChild(self.current).execute(delta=delta);
		if ret is None:
			return None
		elif ret == False:
			return False
