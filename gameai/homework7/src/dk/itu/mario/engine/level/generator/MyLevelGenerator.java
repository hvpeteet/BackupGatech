...
public class MyLevelGenerator{
	...
	private static final int NUM_CROSS = 0;
	private static final int MIN_ITERATIONS = 1000;
	private static final int MAX_ITERATIONS = 10000;
	private static final int MUTATIONS_PER_ITERATION = 5;
	private static final int GENES_PER_INDIVIDUAL = 40;
	private static final double GOAL_FITNESS = 0.80;
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		individual = MyDNA.makeRandom(MyLevelGenerator.GENES_PER_INDIVIDUAL);
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		if (count > MyLevelGenerator.MAX_ITERATIONS) {
			return true;
		} else if (count > MyLevelGenerator.MIN_ITERATIONS && getBestIndividual(population).getFitness() >= MyLevelGenerator.GOAL_FITNESS) {
			return true;
		}
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		while (selected.size() < MyLevelGenerator.MUTATIONS_PER_ITERATION) {
			MyDNA to_mutate = population.get(rand.nextInt(population.size()));
			selected.add(to_mutate);
		}
		// YOUR CODE GOES ABOVE HERE
		return selected;
	}
	...
	private int numberOfCrossovers ()
	{
		...
		// YOUR CODE GOES BELOW HERE
		num = MyLevelGenerator.NUM_CROSS;
		// YOUR CODE GOES ABOVE HERE
		return num;
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		ArrayList<MyDNA> finalPopulation = new ArrayList<MyDNA>();
		// YOUR CODE GOES BELOW HERE
		ArrayList<MyDNA> total_pop = new ArrayList<MyDNA>();
		total_pop.addAll(oldPopulation);
		total_pop.addAll(newPopulation);
		Collections.sort(total_pop);
		Collections.reverse(total_pop);
		total_pop = new ArrayList<MyDNA>(total_pop.subList(0, MyLevelGenerator.POP_SIZE));
		finalPopulation = total_pop;
		// YOUR CODE GOES ABOVE HERE
		if (finalPopulation.size() != this.getPopulationSize()) {
			System.err.println("Population not the correct size.");
