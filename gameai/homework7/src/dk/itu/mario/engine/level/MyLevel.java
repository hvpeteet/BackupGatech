...
public class MyLevel extends Level{
	...
	private static final int STARTOFFSET = 8;
	private static final int EXITOFFSET = 8;
	...
	public void create(MyDNA dna, int type)
	{
        ...
        int cur_x = STARTOFFSET;
        // remember lower heights are higher on the screen
        int cur_y = DEFAULTHEIGHT-2;
        while (cur_x < width-EXITOFFSET) {
    		for (Gene g : dna.genes) {
                if (cur_x >= width-EXITOFFSET) {
                    break;
                }
                switch(g.type) {
                    case HILL:
                        int levels = g.param;
                        setBlock(cur_x, DEFAULTHEIGHT-2, HILL_TOP);
                        setBlock(cur_x, DEFAULTHEIGHT-1, GROUND);
                        cur_x += 1;
                        for (int y = 0; y < levels; y++) {
                            for (int x = y; x < levels - y + 3; x++) {
                                setBlock(cur_x + x, cur_y - y - 1, HILL_TOP);
                                setBlock(cur_x + x, DEFAULTHEIGHT-1, GROUND);
                                setBlock(cur_x + x, DEFAULTHEIGHT-2, HILL_TOP);
                            }
                        }
                        cur_x += levels + 3;
                        setBlock(cur_x, DEFAULTHEIGHT-2, HILL_TOP);
                    ...
                    case GAP:
                        ...
                        break;
                    case PLATFORM:
                        int platform_length = g.param;
                        for (int x = 0; x < platform_length; x++) {
                            setBlock(cur_x + x, DEFAULTHEIGHT-5, BLOCK_EMPTY);
                        }
                        break;
                    case COINS:
                        int num_coins = g.param;
                        for (int x = 0; x < num_coins; x++) {
                            setBlock(cur_x + x, DEFAULTHEIGHT-3, COIN);
                            setBlock(cur_x + x, DEFAULTHEIGHT-2, HILL_TOP);
                            setBlock(cur_x + x, DEFAULTHEIGHT-1, GROUND);
                        }
                        cur_x += num_coins;
                        break;
                    case GOOMBAS:
                        int num_goombas = g.param;
                        for (int x = 0; x < num_goombas; x++) {
                            setSpriteTemplate(cur_x + x, DEFAULTHEIGHT-3, new SpriteTemplate(Enemy.ENEMY_GOOMBA, false)); // goombas should not fly
                            setBlock(cur_x + x, DEFAULTHEIGHT-2, HILL_TOP);
                            setBlock(cur_x + x, DEFAULTHEIGHT-1, GROUND);
                        }
                        cur_x += num_goombas;
                        break;
                    case KOOPAS:
                        int num_koopas = g.param;
                        for (int x = 0; x < num_koopas; x++) {
                            setSpriteTemplate(cur_x + x, DEFAULTHEIGHT-3, new SpriteTemplate(Enemy.ENEMY_GREEN_KOOPA, false)); // goombas should not fly
                            setBlock(cur_x + x, DEFAULTHEIGHT-2, HILL_TOP);
                            setBlock(cur_x + x, DEFAULTHEIGHT-1, GROUND);
                        }
                        cur_x += num_koopas;
