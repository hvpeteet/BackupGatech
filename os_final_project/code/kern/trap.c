...
trap_init(void)
{
  ...
  SETGATE(idt[44], 0, GD_KT, int44, 0);
  SETGATE(idt[45], 0, GD_KT, int45, 0);
  SETGATE(idt[46], 0, GD_KT, int46, 0);
  SETGATE(idt[47], 0, GD_KT, int47, 0);
  SETGATE(idt[48], 0, GD_KT, int48, 0);
  
  SETGATE(idt[T_SYSCALL], 0, GD_KT, intSyscall, 3);
  // Per-CPU setup
  trap_init_percpu();
}
...
{
  //cprintf("Entering TRAP dispatch TRAP = %d (%s)\n", tf->tf_trapno, trapname(tf->tf_trapno));
  switch(tf->tf_trapno) {
    case T_DEBUG:
      tf->tf_eflags &= ~(0x100); // Turn of TF flag
      monitor(tf);
      break;
    case T_PGFLT:
...
{
  // if ((tf->tf_es & 0x3) == 3) { // Check if priviledge level was kernel or user.
  //   lock_kernel();
  // }
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    // cprintf("FAULT: va=%p, pte=0x%x\n", fault_va, *pte);
    if (*pte & PTE_P) {
      panic("page present but fault\n");
    }   
    if (*pte & PTE_ON_DISK && PAGING_ENABLED) {
      page_fault_intercept(curenv->env_pgdir, fault_va);
      return;
    }
    ...
    if (NULL != upcall) {
        uintptr_t next_tf_addr;
        // Determine if we came from another exception
        if (UXSTACKTOP - 1 >= tf->tf_esp && tf->tf_esp >= UXSTACKTOP - PGSIZE) {
            // Came from exception stack
            next_tf_addr = tf->tf_esp - sizeof(struct UTrapframe) - 4;
        } else {
            // Came from user stack
            next_tf_addr = UXSTACKTOP - sizeof(struct UTrapframe);
        }
        user_mem_assert(curenv, (void*) next_tf_addr, 1, PTE_W);

        struct UTrapframe* newFrame = (void*) next_tf_addr;

        newFrame->utf_fault_va = fault_va;
        newFrame->utf_err = tf->tf_err;
        newFrame->utf_regs = tf->tf_regs;
        newFrame->utf_eip = tf->tf_eip;
        newFrame->utf_eflags = tf->tf_eflags;
        newFrame->utf_esp = tf->tf_esp;

        curenv->env_tf.tf_eip = (uintptr_t) upcall;
        curenv->env_tf.tf_esp = next_tf_addr;
        env_run(curenv);
    }
    //
