...
// the caller in ipc->readRet, then update the.  Returns
...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	struct OpenFile* f;
	int err = openfile_lookup(envid, req->req_fileid, &f);
	if (err) {
		return err;
	}
    int num_read = file_read(f->o_file, ret->ret_buf, req->req_n, f->o_fd->fd_offset);
    if (num_read < 0) {
    	return num_read;
    }
    f->o_fd->fd_offset += num_read;
	return num_read;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int num_written = file_write(o->o_file, req->req_buf, req->req_n, o->o_fd->fd_offset);
	if (num_written > 0) {
		o->o_fd->fd_offset += num_written; 
	}
	return num_written;
...
serve(void)
{
	...
	while (1) {
		perm = 0;
		// cprintf("file system server waiting on ipc\n");
		req = ipc_recv((int32_t *) &whom, fsreq, &perm);
		// cprintf("file sytem server recieved ipc\n");
		if (debug)
		...
		// cprintf("sending response to ipc from file system serve\n");
