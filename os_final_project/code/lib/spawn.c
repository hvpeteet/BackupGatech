...
{
  uintptr_t pg_index;
  for (pg_index = 0; pg_index < USTACKTOP; pg_index += PGSIZE) {
    if ((uvpd[PDX(pg_index)] & PTE_P) &&
       (uvpt[PGNUM(pg_index)] & PTE_P) &&
       (uvpt[PGNUM(pg_index)] & PTE_U) &&
       (uvpt[PGNUM(pg_index)] & PTE_SHARE))
    {
      sys_page_map(0, (void*)pg_index, child, (void*)pg_index, (uvpt[PGNUM(pg_index)] & PTE_SYSCALL));
    }
  }
  return 0;
}
