...
public class State {
    ...
    public Float getFloat(String key) {
        Float ret;
        if (peek().get(key) instanceof java.lang.Integer) {
            ret = (float) ((int)peek().get(key));
        } else {
            ret = (Float)peek().get(key);
        }
        if (ret == null) {
            if (global.get(key) instanceof java.lang.Integer) {
                ret = (float) ((int)global.get(key));
            } else {
                ret = (Float)global.get(key);
            }
        }
        return (ret == null)
            ? 0
    ...
    // Returns the state
    public State setInt(boolean local, String key, int value) {
    ...
    // Set local to true to bind to top frame
    // Returns the state
    public State setFloat(boolean local, String key, float value) {
        (local ? peek() : global).set(key, value);
        return this;
    }
    ...
    // Set local to true to bind to top frame
    // Returns the state
    public State setString(boolean local, String key, String value) {
        (local ? peek() : global).set(key, value);
        return this;
    }
    ...
    // Returns the state
    public State initIntArray(boolean local, String key, int size, Integer defaultValue) {
    ...
    //    b. If local is true, inserts into top frame
    // Returns the state
    public State initFloatArray(boolean local, String key, int size, Float defaultValue) {
    ...
    // Throws an exception if no such array exists
    public State storeIntArray(String key, int index, int value) throws Exception {
        Integer address = getInt(key);
        if (address == null) {
    ...
    // Throws an exception if no such array exists
    public State storeFloatArray(String key, int index, float value) throws Exception {
        Integer address = getInt(key);
        if (address == null) {
            throw new Exception("Cannot store into nonexistent array");
        } else {
    ...
    public State storeFloat(int address, int offset, float value) {
        mem.setFloat(address, offset, value);
        return this;
    }

    public State storeInt(int address, int offset, int value) {
        mem.setInt(address, offset, value);
        return this;
    }

    public float loadFloat(int address, int offset) {
        return mem.getFloat(address, offset);
    }

    public int loadInt(int address, int offset) {
        return mem.getInt(address, offset);
    }
